...
CXX := g++
CXXFLAGS := -Wl,--no-as-needed -std=c++11 -pthread -g -I$(HEADER_DIR)#-mcmodel=medium
...
node_main : $(OBJ_DIR)/node_main.o $(OBJ_DIR)/tcp_node.o $(OBJ_DIR)/tcp_server.o $(OBJ_DIR)/tcp_client.o /usr/local/lib/libsockpp.so $(OBJ_DIR)/memory_pool.o /usr/lib/libisal.so $(OBJ_DIR)/task_generator.o
	$(CXX) $(CXXFLAGS) $^ -o $(BIN_DIR)/$@
